// idc_tools DICOM decode worker (classic worker). Fetches DICOM straight from the open-CORS IDC AWS
// buckets and reconstructs a volume (+ optional SEG labelmap) off the main thread. Returns transferable
// typed arrays. Ported from the SlicerLive SEG Roulette loader.
//
// Message in:  { ctKeys, segKeys, ctBucket, segBucket, modality }
// Messages out: ctinfo | thumb | seg | progress | ct | labelmap | alldone | error
declare const dcmjs: any;
...
(function loadDcmjs() {
  ...
  for (let i = 0; i < 12; i++) { try { importScripts(mirrors[i % mirrors.length]); return; } catch (e) { /* try next */ } }
  throw new Error('dcmjs: all CDN mirrors failed');
})();

const s3url = (b?: string) => 'https://' + (b || 'idc-open-data') + '.s3.us-east-1.amazonaws.com/';
let CT_S3 = s3url(), SEG_S3 = s3url();
let MODNAME = 'image';
const post = (m: any, x?: Transferable[]) => (self as any).postMessage(m, x || []);
const prog = (msg: string, frac: number) => post({ t: 'progress', msg, frac });
...
function naturalize(buf: ArrayBuffer) {
...
const lps2ras = (v: number[]) => [-v[0], -v[1], v[2]]; // DICOM LPS -> RAS

async function fetchBuf(key: string, base?: string) { return (await fetchRetry((base || CT_S3) + key)).arrayBuffer(); }

function makeThumb(ds: any) {
...
async function buildVolume(ctKeys: string[]) {
  ...
  const c0 = lps2ras(rowDir.map((v: number) => v * ps[1])), c1 = lps2ras(colDir.map((v: number) => v * ps[0])),
        c2 = lps2ras(normal.map((v: number) => v * sliceSpacing)), o = lps2ras(p0);
  const ijkToRAS = [c0[0], c1[0], c2[0], o[0], c0[1], c1[1], c2[1], o[1], c0[2], c1[2], c2[2], o[2], 0, 0, 0, 1];
  const isPET = MODNAME === 'PET';
  const vol: Int16Array | Float32Array = isPET ? new Float32Array(nx * ny * nz) : new Int16Array(nx * ny * nz);
  ...
  if (MODNAME === 'CT') {
    ...
    const N = vol.length, step = Math.max(1, (N / 200000) | 0), samp: number[] = [];
    for (let i = 0; i < N; i += step) { const v = vol[i]; if (!isPET || v > 0) samp.push(v); }
    samp.sort((a, b) => a - b);
    const pct = (f: number) => (samp.length ? samp[Math.min(samp.length - 1, (f * samp.length) | 0)] : 0);
...
function buildLabelmap(ds: any, bits: Uint8Array, ct: any) {
  ...
  const colors: number[][] = [], names: Record<number, string> = {};
...
async function fetchSeg(key: string) {
  const HEAD = 4 << 20;
  const head = new Uint8Array(await fetchRetry(SEG_S3 + key, { headers: { Range: `bytes=0-${HEAD - 1}` } }).then((r) => r.arrayBuffer()) as ArrayBuffer);
  ...
  if (have > 0) bits.set(head.subarray(valOff, valOff + have), 0);
...
(self as any).onmessage = async (e: MessageEvent) => {
  ...
  CT_S3 = s3url(ctBucket); SEG_S3 = s3url(segBucket);
  MODNAME = ({ CT: 'CT', MR: 'MR', PT: 'PET' } as Record<string, string>)[modality] || 'image';
  try {
    ...
    post({ t: 'alldone' });
  } catch (err: any) { post({ t: 'error', error: String(err && err.stack || err) }); }
};
